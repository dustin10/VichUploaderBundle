...
use Vich\UploaderBundle\Tests\Fixtures\NonCloneableHashNamer;
...
final class PropertyMappingResolverConfigurationTest extends TestCase
{
    ...
    public function testCustomSubclassesAreNotImplicitlyCloned(): void
    {
        $namer = new NonCloneableHashNamer();
        $resolver = $this->createNamerResolver($namer, ['length' => 8], []);
        $object = self::createObject();
        $mapping = $resolver->resolve($object, 'file', ['mapping' => 'configured']);

        self::assertSame($namer, $mapping->getNamer());
        self::assertMatchesRegularExpression('/^[[:xdigit:]]{8}\.txt$/', $mapping->getNamer()->name($object, $this->createFileMapping()));
    }
     ...
     */
    private function createFileMapping(string $originalName = 'Fôo Bàr.txt'): PropertyMapping
    {
